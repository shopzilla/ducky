...
-->
    ...
    <xsl:template match="/wadl:application">   
        <html>
            <head>
                ...
                <link rel="shortcut icon" href="http://img01.shopzilla-images.com/sz2s/sz2/common/images/shopzilla.ico" />
                <style type="text/css">
                    body {
                ...
                <script type="text/javascript" src="/services/catalog/v1/js/ui.js" />
            ...
            <body>
                ...
                <p id="footer">Shopzilla and Bizrate are servicemarks of Shopzilla, Inc. (C) 1996-2009. All rights reserved.</p>
            </body>
        </html>
    </xsl:template>
    ...
    <xsl:template match="wadl:*" mode="form-param-group">
        ...
        <div class="testFormBox">
          <a id="link-{$id}" class="formToggle" href="javascript:toggleVisibility('link-{$id}', 'form-{$id}');">[ + ] show test form</a>
          ...
          <form class="testForm" id="form-{$id}" style="visibility: hidden; display: none">
            ...
            <xsl:choose>
              ...
              <xsl:otherwise><xsl:attribute name="method">GET</xsl:attribute></xsl:otherwise>
            </xsl:choose>
            <h5><xsl:value-of select="$context"/></h5>
            ...
            <table>
              ...
              <xsl:apply-templates select="ancestor-or-self::wadl:*/wadl:param[@style=$style]" mode="form-param"/>
            </table>
            <input type="submit"/>
          </form>
        </div>
                ...
                <xsl:choose>
                    <xsl:when test="wadl:option">
                        <select>
                            ...
                            <xsl:choose>
                              ...
                              <xsl:otherwise><option value="">...</option></xsl:otherwise>
                            </xsl:choose>
                    ...
                    <xsl:otherwise>
                        <input type="text">
                          ...
                          <xsl:if test="@fixed"><xsl:attribute name="value"><xsl:value-of select="@fixed"/></xsl:attribute></xsl:if>
                        </input>
                    </xsl:otherwise>
                </xsl:choose>                        
            </td>
    ...
    <xsl:template match="wadl:option" mode="form-param-option">
        <option>
            ...
            <xsl:if test="ancestor::wadl:param[1]/@default=@value"> <xsl:attribute name="selected" /></xsl:if>
    ...
    <xsl:template name="get-element">
        ...
        <p><em>Source: <a><xsl:attribute name="href"><xsl:value-of select="substring-after($source, 'xsd/')"/></xsl:attribute><xsl:value-of select="substring-after($source, 'xsd/')"/></a></em></p>
